...
import { Plus, Pencil, Trash2, Pill, Lock } from "lucide-react";
import { toast } from "sonner";

export default function GerenciarMedicamentos() {
  ...
  const { data: opcoes, isLoading } = trpc.opcoesMedicamentos.list.useQuery();
  
  const createMutation = trpc.opcoesMedicamentos.create.useMutation({
    onSuccess: () => {
      toast.success("Medicamento adicionado com sucesso!");
      utils.opcoesMedicamentos.list.invalidate();
      resetForm();
    },
    onError: (error) => {
  ...
  const updateMutation = trpc.opcoesMedicamentos.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = trpc.opcoesMedicamentos.delete.useMutation({
    onSuccess: () => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Adicione, edite ou remova opções de medicamentos que aparecem no cadastro de gestantes e no cartão de pré-natal.
              </CardDescription>
            </div>
                ...
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {editingId 
                      ...
                      : "Preencha as informações do novo medicamento."}
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="permiteTextoLivre">
                      Permite especificação adicional
                    </Label>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Quando ativado, permite que o usuário adicione uma especificação personalizada (ex: dosagem, frequência) ao selecionar este medicamento.
                  </p>
                </div>
                <DialogFooter>
