...
async function main() {
  ...
  try {
    // Buscar gestante TESTE TESTE para pegar os dados
    ...
    console.log(`✅ Gestante encontrada: ${gestante.nome}`);
    ...
    // E-mail do proprietário da conta (único que funciona no modo teste)
    const destinatario = 'prenatalmaismulher@gmail.com';
...
<body>
  <div class="container">
    ...
    const result = await resend.emails.send({
      from: 'Mais Mulher <onboarding@resend.dev>',
      to: destinatario,
      subject: '[TESTE] Lembrete: Vacina Bronquiolite - Gestante TESTE TESTE',
      html: htmlContent,
    });
    ...
    await connection.execute(
      `INSERT INTO logsEmails (gestanteId, tipoLembrete, emailDestinatario, assunto, corpo, status, createdAt)
       VALUES (?, ?, ?, ?, ?, ?, NOW())`,
      [gestante.id, 'bronquiolite_teste', destinatario, '[TESTE] Lembrete: Vacina Bronquiolite', htmlContent, 'enviado']
    );
