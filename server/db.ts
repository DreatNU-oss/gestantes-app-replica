...
// Limites dinâmicos:
// - Até 34 semanas: 40 dias
// - 34-36 semanas: 18 dias
// - Após 36 semanas: 10 dias
function getLimiteDiasConsulta(igSemanas: number | null): { limite: number; faixa: string } {
  ...
  if (igSemanas < 34) {
    ...
    return { limite: 18, faixa: '34-36 semanas' };
  } else {
    return { limite: 10, faixa: 'Após 36 semanas' };
  }
}
...
export async function getGestantesSemConsultaRecente(): Promise<{
  ...
  const gestantesComJustificativa = new Set(
    justificativasData
      .filter(j => {
        // Justificativas temporárias: "ja_agendada", "consulta_apos_morfologico" e "espaco_maior_consultas"
        if (j.motivo === 'ja_agendada' || j.motivo === 'consulta_apos_morfologico' || j.motivo === 'espaco_maior_consultas') {
